...
function SindicoInfracoes({ infracoes, setInfracoes, moradores, api, toast }) {
    ...
    async function criarInfracao(e) {
        e.preventDefault();
        if (!motivoInfracao.trim() || !moradorInfracao || !dataInfracao) return;
        ...
        <>
                ...
                <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-color)", borderRadius: "12px", padding: "20px", marginBottom: "20px" }}>
                    ...
                    <form onSubmit={criarInfracao} style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                                ...
                                <div style={{ display: "flex", gap: "8px" }}>
                                    <button type="button" onClick={() => setTipoInfracao("MULTA")} style={{
                                    ...
                                    <button type="button" onClick={() => setTipoInfracao("ADVERTENCIA")} style={{
                                        flex: 1, padding: "10px", borderRadius: "8px", cursor: "pointer",
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                                ...
                                <select value={moradorInfracao} onChange={(e) => setMoradorInfracao(e.target.value)} required
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                                ...
                                <input type="text" value={motivoInfracao} onChange={(e) => setMotivoInfracao(e.target.value)} placeholder="Ex: Barulho após 22h" required
                                    style={{ padding: "10px", borderRadius: "8px", border: "1px solid var(--border-color)", background: "var(--bg-input, var(--bg-card))", color: "var(--text-primary)", fontSize: "0.9rem" }} />
                            </div>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                                ...
                                <DatePicker selected={dataInfracao} onChange={(date) => setDataInfracao(date)} locale="pt-BR" dateFormat="dd/MM/yyyy"
                                    maxDate={new Date()} placeholderText="Selecione a data" className="input-datepicker" required />
                            </div>
                            ...
                            {tipoInfracao === "MULTA" && (
                                <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                                    ...
                                    <input type="number" step="0.01" min="0" value={valorInfracao} onChange={(e) => setValorInfracao(e.target.value)} placeholder="Ex: 150.00"
                                        style={{ padding: "10px", borderRadius: "8px", border: "1px solid var(--border-color)", background: "var(--bg-input, var(--bg-card))", color: "var(--text-primary)", fontSize: "0.9rem" }} />
                                </div>
                            )}
                        </div>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                            ...
                            <textarea value={descricaoInfracao} onChange={(e) => setDescricaoInfracao(e.target.value)} placeholder="Detalhes sobre a infração..." rows={3}
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", margin: "0 0 16px" }}>
                {["TODOS", "MULTA", "ADVERTENCIA"].map(t => (
                    <button key={t} onClick={() => setFiltroTipoInf(t)} style={{
                        ...
                        cursor: "pointer", fontSize: "0.85rem", fontWeight: filtroTipoInf === t ? "600" : "400"
                    }}>
                ...
                {["TODOS", "PENDENTE", "CONTESTADA", "PAGA", "CANCELADA"].map(s => (
                    <button key={s} onClick={() => setFiltroStatusInf(s)} style={{
                        padding: "6px 14px", borderRadius: "20px",
                        ...
                        cursor: "pointer", fontSize: "0.85rem", fontWeight: filtroStatusInf === s ? "600" : "400"
                    }}>
                        ...
                        <div key={inf.id} style={{
                            ...
                            <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                                {inf.status !== "PAGA" && inf.tipo === "MULTA" && (
                                    <button onClick={() => alterarStatusInfracao(inf.id, "PAGA")} style={{
                                        padding: "6px 14px", borderRadius: "6px", border: "1px solid #2ecc71",
                                        background: "rgba(46,204,113,0.1)", color: "#2ecc71", cursor: "pointer", fontSize: "0.8rem", fontWeight: "500"
                                    }}>Marcar como Paga</button>
                                )}
                                {inf.status !== "CANCELADA" && (
                                    <button onClick={() => alterarStatusInfracao(inf.id, "CANCELADA")} style={{
                                        padding: "6px 14px", borderRadius: "6px", border: "1px solid #95a5a6",
                                        background: "rgba(149,165,166,0.1)", color: "#95a5a6", cursor: "pointer", fontSize: "0.8rem", fontWeight: "500"
                                    }}>Cancelar</button>
                                )}
                                {inf.status === "CONTESTADA" && (
                                    <button onClick={() => alterarStatusInfracao(inf.id, "PENDENTE")} style={{
                                        padding: "6px 14px", borderRadius: "6px", border: "1px solid #f1c40f",
                                        background: "rgba(241,196,15,0.1)", color: "#f1c40f", cursor: "pointer", fontSize: "0.8rem", fontWeight: "500"
