...
function SindicoManutencoes({ perfil, api, toast, formatarData }) {
    ...
    async function criarManutencao(e) {
        e.preventDefault();
        if (!dataInicio) {
            toast.erro("Selecione a data de início.");
            return;
        }
                            ...
                            >
                                ...
                                {(m.status === "AGENDADA" || m.status === "EM_ANDAMENTO") && (
                                    <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                                        {m.status === "AGENDADA" && (
                                            <button
                                                onClick={() => pedirConfirmacao(m.id, "EM_ANDAMENTO")}
                                                style={{
                                                    padding: "5px 12px",
                                                    borderRadius: "6px",
                                        ...
                                        {m.status === "EM_ANDAMENTO" && (
                                            <button
                                                onClick={() => pedirConfirmacao(m.id, "CONCLUIDA")}
                                                style={{
                                                    padding: "5px 12px",
                                                    borderRadius: "6px",
                                        ...
                                        <button
                                            onClick={() => pedirConfirmacao(m.id, "DELETAR")}
                                            style={{
                                                padding: "5px 12px",
                                                borderRadius: "6px",
            ...
            {modalConfirm.aberto && (
                <div className="modal-overlay" onClick={() => setModalConfirm({ aberto: false, id: null, acao: null })}>
                    <div className="modal-confirm" onClick={e => e.stopPropagation()}>
                        <div className="modal-confirm-icone">{modalConfirm.acao === "DELETAR" ? "" : ""}</div>
